...
snowcover_data_fixed <- snowcover_data_fixed |> 
  mutate(Date = case_when(
    Date == "8&9 june 06" ~ "09-June-06", .default = Date
  )) |> 
  mutate(Date2 = as_date(Date))
```
...
- if snow_cover is larger than 10%
...
- joins are an expensive calculation in data manipulation
- should aim to simplify the datasets the most before using a join operation

```{r}
nests_snow_month <- nests_month |> 
  inner_join(snowdays_threshold, join_by(Site, month, Year))
```
...
nests_snow_year <- nests_snow_month |> 
  ...
  summarise(cum_snow_days = sum(days, na.rm = TRUE),
            avg_snow_cover = mean(avg_cover, na.rm = TRUE),
            n_nest = sum(n_nest))
```
